...
describe("BankAccount", function () {
  ...
  async function deployBankAccountWithAccounts(
    ...
    if (deposit > 0) {
      await bankAccount.connect(addr0).deposit(0, {
        value: deposit,
      });
    }
  ...
  describe("Creating an account", () => {
    ...
    it("should not allow creating an account with more than 3 accounts per user", async () => {
      const { bankAccount, addr0 } = await loadFixture(deployBankAccount);
  ...
  describe("Depositing", () => {
    ...
    it("should NOT allow deposit from non-account owner", async () => {
      ...
      await expect(bankAccount.connect(addr1).deposit(0, { value: 100 })).to.be
        .reverted;
    });
  });
  ...
  describe("Withdraw", () => {
    describe("Request a withdraw", () => {
      ...
      it("should allow multiple withdraw requests from the same owner if funds permit", async () => {
        const { bankAccount, addr0 } = await deployBankAccountWithAccounts(
          1,
          100
        );
    ...
    describe("Approve a withdraw", () => {
      it("should allow account owner (other than the requester) to approve withdraw", async () => {
        const { bankAccount, addr1 } = await deployBankAccountWithAccounts(
          2,
          100,
          [100]
    ...
    describe("Make withdraw", () => {
      it("should allow creator of request to withdraw after approval", async () => {
      ...
      it("should not allow creator of request to withdraw an approved request twice", async () => {
        const { bankAccount, addr0, addr1 } =
        ...
        // Attempting the same withdrawal again should fail
      ...
      it("should not allow withdraw before any approval", async () => {
        const { bankAccount, addr0 } = await deployBankAccountWithAccounts(
          2,
          200,
          [100]
